...
elseif (isset($_REQUEST['delete']) && isset($_REQUEST['itemid'])) {
	...
	########################################################################################################
	# delete racktables item
	require_once('racktablesapi.php');
	if($result) {
		$response = deleteItem(array($_REQUEST['itemid']));

		if(isset($response['error'])) {
			show_messages(false, '', 'Deleting racktable item is failed. '. $response['error']);
		}
	}
	########################################################################################################
...
elseif (isset($_REQUEST['save'])) {
	...
	if ($result) {
		...
		if (hasRequest('itemid')) {
			...
			#################################################################################
			# update racktables item
			require_once('racktablesapi.php');
			$item['itemid'] = $itemId;
			$response = updateItem($item);

			if(isset($response['error'])) {
				show_messages(false, '', 'Updating racktable item is failed. '. $response['error']);
			}
			#################################################################################
			...
			$item = API::Item()->get(array(
				'output' => 'extend',
				'hostids' => get_request('hostid'),
				'search' => array('key_' => get_request('key'))
			));
			$item = reset($item);

			$response = addItem($host['name'], $item);

			if(isset($response['error'])) {
				show_messages(false, '', 'Adding racktable item is failed.'. $response['error']);
			}
			#################################################################################
...
elseif (isset($_REQUEST['update']) && isset($_REQUEST['massupdate']) && isset($_REQUEST['group_itemid'])) {
	...
	if($result) {
		foreach($_REQUEST['group_itemid'] as $id) {
			...
			$itemToUpdate = API::Item()->get(array(
				'output' => 'extend',
				'itemids' => $id,
			));
...
elseif (str_in_array(getRequest('go'), array('activate', 'disable')) && hasRequest('group_itemid')) {
	...
	###################################################################################
	# update status of racktables item 
	require_once('racktablesapi.php');
	// status( 0:有効、1:無効)
	$status = (getRequest('go') != 'activate');
	$itemids = array();
	if(is_array($groupItemId)) {
		$itemids = array_values($groupItemId);
	} else {
		array_push($itemids, $groupItemId);
	}
	$response = updateItemStatus($itemids, $status);
	if(isset($response['error'])) {
		show_messages(false, '', 'Updating racktables item is failed. ' . $response['error']);
	}
	# END
	###################################################################################
...
elseif ($_REQUEST['go'] == 'delete' && isset($_REQUEST['group_itemid'])) {
	...
	require_once('racktablesapi.php');
	if($goResult) {
		$response = deleteItem(array_keys($group_itemid));
		if(isset($response['error'])) {
			show_message(false, '', 'Deleting racktable item is failed. '. $response['error']);
		}
	}
	######################################################################################
