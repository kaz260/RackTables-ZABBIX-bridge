...
elseif (isset($_REQUEST['save'])) {
	...
	$hosts = API::Host()->get(array(
		...
		###################################################################
		# get the name
		# 'output' => array('hostid')→'output' => array('hostid', 'name'),
	...
	if (!empty($_REQUEST['groupid'])) {
		...
		$response = updateRack($oldGroup['name'], $_REQUEST['name'], $hostnames);
		if(isset($response['error'])) {
			show_messages(false, '', 'Updating racktables Rack is failed.');
		}
		#############################################################################################################
...
elseif (isset($_REQUEST['delete']) && isset($_REQUEST['groupid'])) {
	...
	$response = deleteRack($group['name']);
	if(isset($response['error'])) {
		show_messages(false, '', 'Deleting racktables Rack is failed.');
	}
	#############################################################################################################
...
elseif ($_REQUEST['go'] == 'delete') {
	...
	foreach($groupids as $groupid) {
		...
		if(isset($response["error"])) {
			show_messages(false, '', 'Deleting racktables rack is failed. Error message:' . $response['error']);
		}
	}
	######################################################################################################
...
elseif (str_in_array(getRequest('go'), array('activate', 'disable'))) {
	...
	if ($groups) {
		...
		if($result) {
			require_once('racktablesapi.php');
			$hostNames = zbx_objectValues($hosts, 'name');
