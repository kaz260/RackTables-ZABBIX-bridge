...
elseif (isset($_REQUEST['go']) && $_REQUEST['go'] == 'massupdate' && isset($_REQUEST['masssave'])) {
	...
	try {
		...
		#################################################
		# get the host name and status
		# 'output' => array('hostid') →　'output' => array('hostid', 'name', 'status')
		#################################################
		...
		if (isset($visible['new_groups']) && !empty($_REQUEST['new_groups'])) {
			foreach($_REQUEST['new_groups'] as $newGroup) {
				...
				if (is_array($newGroup) && isset($newGroup['new'])) {
					# new group
					$response = addRack($newGroup['new']);
					if(isset($response['error'])) {
						show_messages(false, '', 'Adding racktables rack is failed. Error message：'. $response['error']);
					} else {
						array_push($groupnames, $newGroup['new']);
					}
				} else {
					$group = API::HostGroup()->get(array(
						'groupids' => $newGroup,
		...
		if (isset($visible['groups']) && !empty($_REQUEST['groups'])) {
		...
		foreach($hosts['hosts'] as $host) {
			$hostname = $host['name'];
			$has_problems = isset($_REQUEST['status']) ? $_REQUEST['status'] : $host['status'];
			$response = updateObject($host['name'], $host['name'], $groupnames, $has_problems);
			if(isset($response['error'])) {
				show_messages(false, '', "Updating racktables object $hostname is failed. Error message：" . $response['error']);
			}
		}
		########################################################################################################
