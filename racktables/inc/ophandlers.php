...
function topUpdateObjectAllocation() {
	...
	assertUIntArg("type", false);

	global $dbxlink;
	$dbxlink->beginTransaction();
	// object
	try {
		$object_id = commitAddObject ($_REQUEST["name"], NULL, $_REQUEST["type"], '', array());
	...
	// object ip
	if(strlen($_REQUEST['agentiplist']) > 0) {
		$agentips = explode ("\n", $_REQUEST['agentiplist']);
		$agentips = bindIP($agentips, $object_id);
	}
	if(strlen($_REQUEST['snmpiplist']) > 0) {
		$snmpips = explode ("\n", $_REQUEST['snmpiplist']);
		$snmpips = bindIP($snmpips, $object_id);
	}
	if(strlen($_REQUEST['jmxiplist']) > 0) {
		$jmxips = explode ("\n", $_REQUEST['jmxiplist']);
		$jmxips = bindIP($jmxips, $object_id);
	}
	if(strlen($_REQUEST['impiiplist']) > 0) {
		$ipmiips = explode ("\n", $_REQUEST['impiiplist']);
		$ipmiips = bindIP($ipmiips, $object_id);
	}	
	...
	// zabbix host
	$added_flag = true;
	$result = addHostWithInterfaces($object['name'], $agentips, $snmpips, $jmxips, $ipmiips);
	...
	// zabbix host group
	$allracks = getMoleculeForObject($object['id']);
	$rackids = array();
	$racknames = array();
	...
	if(isset($result['error'])) {
		showError('Setting zabbix hostgroup for host is failed. Error message：' . $result['error']);
	}


	$left_params = array('page', 'tab', 'op', 'rackmulti');
...
$msgcode['bindIP']['OK'] = 48;
$msgcode['bindIP']['ERR1'] = 170;
...
function updateObjectItems() {
	if (isset($_REQUEST['Save'])) {
		...
		$item = array(
			...
			'history' => isset($_REQUEST['history']) ? $_REQUEST['history'] : 90,
			'status' => isset($_REQUEST['status']) ? $_REQUEST['status'] : 0,
			'type' => isset($_REQUEST['type']) ? $_REQUEST['type'] : null,
			'snmp_community' => $_REQUEST['snmp_community'],
			'snmp_oid' => $_REQUEST['snmp_oid'],
			'value_type' => isset($_REQUEST['value_type']) ? $_REQUEST['value_type'] : 0,
			'trapper_hosts' => $_REQUEST['trapper_hosts'],
			'port' => isset($_REQUEST['port']) ? $_REQUEST['port'] : null,
			'units' => $_REQUEST['units'],
			'multiplier' => isset($_REQUEST['multiplier']) ? $_REQUEST['multiplier'] : 0,
			'delta' => isset($_REQUEST['delta']) ? $_REQUEST['delta'] : 0,
			'snmpv3_contextname' => $_REQUEST['snmpv3_contextname'],
			'snmpv3_securityname' => $_REQUEST['snmpv3_securityname'],
			'snmpv3_securitylevel' => isset($_REQUEST['snmpv3_securitylevel']) ? $_REQUEST['snmpv3_securitylevel'] : 0,
			'snmpv3_authprotocol' => isset($_REQUEST['snmpv3_authprotocol']) ? $_REQUEST['snmpv3_authprotocol'] : 0,
			'snmpv3_authpassphrase' => $_REQUEST['snmpv3_authpassphrase'],
			'snmpv3_privprotocol' => isset($_REQUEST['snmpv3_privprotocol']) ? $_REQUEST['snmpv3_privprotocol'] : 0,
			'snmpv3_privpassphrase' => $_REQUEST['snmpv3_privpassphrase'],
			'formula' => isset($_REQUEST['formula']) ? $_REQUEST['formula'] : 0,
			'trends' => isset($_REQUEST['trends']) ? $_REQUEST['trends'] : 365,
			'logtimefmt' => $_REQUEST['logtimefmt'],
			//'valuemapid' => $_REQUEST['valuemapid'],
			'delay_flex' => $delay_flexStr,
			'authtype' => isset($_REQUEST['authtype']) ? $_REQUEST['authtype'] : 0,
			...
			'data_type' => isset($_REQUEST['data_type']) ? $_REQUEST['data_type'] : 0,
			'inventory_link' => isset($_REQUEST['inventory_link']) ? $_REQUEST['inventory_link'] : 0,
		);
...
function addIPAllocation ()
{
	...
	if(isset($result['error'])) {
		showError('Adding zabbix interface is failed. Error message：'. $result['error']);
	}
	# END
	###############################################################################################
...
function updateObject ()
{
	...
	###############################################################################################
	# update zabbix host
	$status = isCheckSet ('object_has_problems', 'yesno') == 'yes' ? 1 : 0;
	$result = updateHostName($object['dname'], $_REQUEST['object_name'], $status);
	if(isset($result['error'])) {
		showError('Updating zabbix host is failed. Error message：' . $result['error']);
	}
	# END
	###############################################################################################
...
function deleteObject ()
{
	...
	###############################################################################################
	# delete zabbix host
	$result = deleteHost($oinfo['dname']);
	if(isset($result['error'])) {
		showError('Deleting zabbix host is failed. Error message：' . $result['error']);
	}

	// delete items in the Object from DB
	usePreparedDeleteBlade ('item_information', array ('objectid' => $_REQUEST['object_id']));
...
function resetObject ()
{
	...
	if(isset($result['error'])) {
		showError('Resetting zabbix host is failed. Error message：' . $result['error']);
	}
	# END
	###############################################################################################
...
function deleteRow ()
{
	...
	###################################################################################################
	# delete row size information from DB
	usePreparedDeleteBlade ('room_size', array ('row_id' => $_REQUEST['row_id']));
	# END
	###################################################################################################
