...
function getPosition($row_id) {
	$query = "select rack_id,pos_x,pos_y,pos_z from rack_position where row_id = '$row_id'";
	$result = usePreparedSelectBlade($query);
	$positions = array();
	foreach($result as $key => $value) {
		$positions[$value["rack_id"]] = array(
			'pos_x' => $value['pos_x'], 
			'pos_y' => $value['pos_y'],
			'pos_z' => $value['pos_z']
		);
...
function renderTopEdit () {
	$currentRacks = isset($_REQUEST['rackmulti']) ? $_REQUEST['rackmulti'] : array();
	$object_id = isset($_REQUEST['object_id']) ? $_REQUEST['object_id'] : -1;
	...
	renderRackMultiSelect('rackmulti[]', $allRacksData, $currentRacks);
	...
	$name = $_REQUEST['name'];
	echo "<td><input id=name name=name type=text style=width:300px value=$name></td>";
	#echo "<td><input id=name name=name type=text style=width:300px></td>";
	echo '</tr>';
	...
	printNiftySelect ($typelist, array ('name' => 'type'), $type);
	#printNiftySelect ($typelist, array ('name' => 'type'), 0);
	echo '</td>';
	echo '</tr>';
	
	echo '<tr>';
	echo '<td align=right><label>agent interfaces</label></td>';
	$agentiplist = $_REQUEST['agentiplist'];	
	echo "<td><textarea id=agentiplist name=agentiplist rows=5 cols=40 style=width:300px>$agentiplist</textarea><br></td>\n";
	#echo "<td><textarea id=agentiplist name=agentiplist rows=5 cols=40 style=width:300px></textarea><br></td>\n";
	echo '</tr>';

	echo '<tr>';
	echo '<td align=right><label>SNMP interfaces</label></td>';
	$snmpiplist = $_REQUEST['snmpiplist'];
	echo "<td><textarea id=snmpiplist name=snmpiplist rows=5 cols=40 style=width:300px>$snmpiplist</textarea><br></td>\n";
	#echo "<td><textarea id=snmpiplist name=snmpiplist rows=5 cols=40 style=width:300px></textarea><br></td>\n";
	echo '</tr>';

	echo '<tr>';
	echo '<td align=right><label>JMX interfaces</label></td>';
	$jmxiplist = $_REQUEST['jmxiplist'];
	echo "<td><textarea id=jmxiplist name=jmxiplist rows=5 cols=40 style=width:300px>$jmxiplist</textarea><br></td>\n";
	#echo "<td><textarea id=jmxiplist name=jmxiplist rows=5 cols=40 style=width:300px></textarea><br></td>\n";
	echo '</tr>';

	echo '<tr>';
	echo '<td align=right><label>IMPI interfaces</label></td>';
	$impiiplist = $_REQUEST['impiiplist'];
	echo "<td><textarea id=impiiplist name=impiiplist rows=5 cols=40 style=width:300px>$impiiplist</textarea><br></td>\n";
	#echo "<td><textarea id=impiiplist name=impiiplist rows=5 cols=40 style=width:300px></textarea><br></td>\n";
	echo '</tr>';
	...
	foreach ($currentRacks as $cand_id) {
...
function renderTopEditItems() {
	...
	echo "<div class=dd>";
	$snmpv3_securitylevels = array("other" => array(
		'0' => 'noAuthNoPriv',
		'1' => 'authNoPriv',
		'2' => 'authPriv'));
	...
	*/
	...
	echo "<div class=dd>";
	echo "<div style=width:380px; align=left>";
	$inventory_links = array("other" => array(
		'0' => '-None-',
		'1' => 'Type',
		'2' => 'Type (Full details)',
		'3' => 'Name',
		...
		'13' => 'MAC address B',
		'14' => 'Hardware',
		'15' => 'Hardware (Full details)',
		'16' => 'Software',
		'17' => 'Software (Full details)',
		'18' => 'Software application A',
		'19' => 'Software application B',
		'20' => 'Software application C',
		'21' => 'Software application D',
		'22' => 'Software application E',
		'23' => 'Contact',
		'24' => 'Location',
		'25' => 'Location latitude',
		...
		'42' => 'OOB subnet mask',
		'43' => 'OOB router',
		'44' => 'Date HW purchased',
		'45' => 'Date HW installed',
		'46' => 'Date HW maintenance expires',
		'47' => 'Date HW decommissioned',
		'48' => 'Site address A',
		'49' => 'Site address B',
		'50' => 'Site address C',
		'51' => 'Site city',
		'52' => 'Site state / province',
		'53' => 'Site country',
		'54' => 'Site ZIP / postal',
		'55' => 'Site rack location',
		'56' => 'Site notes',
		'57' => 'Primary POC name',
		'58' => 'Primary POC email',
		'59' => 'Primary POC phone A',
		'60' => 'Primary POC phone B',
		'61' => 'Primary POC cell',
		'62' => 'Primary POC screen name',
		'63' => 'Primary POC notes',
		'64' => 'Secondary POC name',
		'65' => 'Secondary POC email',
		'66' => 'Secondary POC phone A',
		'67' => 'Secondary POC phone B',
		'68' => 'Secondary POC cell',
		'69' => 'Secondary POC screen name',
		'70' => 'Secondary POC notes',));
...
function renderTopAirShiny() {
	...
	$now = new DateTime();
	echo '<iframe width=100% height=800px name=aa frameborder=0 src=http://127.0.0.1:5000/test/?' . $now->format('U') . '></iframe>'; 
...
function renderItem() {
	...
	echo "<select name=objectid style=width:50%; $disabled size=" . getConfigVar ('MAXSELSIZE') . " onchange=getElementsByName(\"viewitems\")[0].submit()>\n";

	$allObjects = scanRealmByText('object');
	$objectlist = array();

	$selected = isset($_REQUEST['objectid']) ? $_REQUEST['objectid'] : -1;
	...
	echo "</select>\n";
	//$object = spotEntity('object', $selected);
	...
	if($selected >= 0) {
		$query .=  " where objectid = '$selected'";
	}
	$query .= ' order by hostid, name';
	$result = usePreparedSelectBlade($query);
	$items = $result->fetchAll();
	unset ($result);

	$count = count($items);
	startPortlet ("Items($count)");
	
	if($count <= 0) {
		echo '<h4>No Item is in the Object.<h4>';
	} else {
		echo '<table style=width:80%; border=0 cellpadding=5 cellspacing=0 align=center class=cooltable>';
		echo '<tr><th>Name</th>';
		echo '<th>Type</th>';
		echo '<th>Key</th>';
		echo '<th>Interface</th>';
		echo '<th>Type of information</th>';
		echo '<th>Data type</th>';
		echo '<th>Units</th>';
		echo '<th>Use custom multiplier</th>';
		echo "<th>Update interval (in sec)</th></tr>\n";

		foreach($items as $item) {
			// Name
			echo "<tr class='row_${order} tdleft ${problem}' valign=top><td><a href=index.php?page=item&tab=properties&item_id=${item['itemid']}><strong>${item['name']}</strong></a></td>\n";
			// Type
			global $itemtypelist;
			echo "<td>${itemtypelist[$item['type']]}</td>";
			// Key
			echo "<td>${item['key_']}</td>";

			// Interface
			$interfaceStr = '-';
			if(isset($item['interfaceid'])) {
				$interface = searchInterfaceById($item['interfaceid'])[0];
				if(isset($interface['ip']))
					$interfaceStr = "${interface['ip']}:${interface['port']}";
			}
			echo "<td>$interfaceStr</td>";

			// Type of information
			global $itemvalue_types;
			echo "<td>${itemvalue_types[$item['value_type']]}</td>";
			// Data type
			global $itemdata_types;
			echo "<td>${itemdata_types[$item['data_type']]}</td>";
			// Uints
			echo "<td>${item['units']}</td>";
			// Use custom multiplier
			if($item['multiplier'] == 1)
				echo "<td>${item['formula']}</td>";
			else
				echo "<td>-</td>";
			// Update interval (in sec)
			echo "<td>${item['delay']}</td></tr>";
		}	
		echo "</table>\n";
	}
