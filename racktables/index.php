...
try {
	...
	case 'interface':
		...
		$allrows = scanRealmByText('row');
		foreach($allrows as $row) {
			if(!isset($tab['top']['default'])) {
				$tab['top']['default'] = $row['name'];
				$tabhandler['top']['default'] = 'renderTop';
				$rowindex['default'] = $row['id'];
			} else {
				$tab['top'][$row['name']] = $row['name'];
				$tabhandler['top'][$row['name']] = 'renderTop';
				$rowindex[$row['name']] = $row['id'];
			}
		}
		
		/*
		// location
		$locations = scanRealmByText('location');
		foreach($locations as $location) {
			if(!isset($tab['top']['default'])) {
				$tab['top']['default'] = $location['name'];
				$tabhandler['top']['default'] = 'renderTop';
				$rowindex['default'] = $location['id'];
			} else {
				$tab['top'][$location['name']] = $location['name'];
				$tabhandler['top'][$location['name']] = 'renderTop';
				$rowindex[$location['name']] = $location['id'];
			}
		}
		// location end
		*/

		$tab['top']['edit'] = 'Add Object';
		$tab['top']['edititems'] = 'Add Item';
		$tab['top']['airshiny'] = 'shiny';
